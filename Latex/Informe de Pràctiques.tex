...
\begin{figure}[h]
	...
	\caption{Força entre els dos fils en funció de la intensitat del corrent al quadrat. Dades de l'ajust lineal: $y=mx+n$, amb $m = (1,0\pm4,5)\cdot 10^{-5}$ N$\cdot$A$^{-2}$ i $n = (0,3\pm4,0)\cdot 10^{-6}$ N. Coeficient de correlació $R^2=0,997$. Notem que en alguns punts les barres d'error corresponent a l'eix de les $x$ són tan petites que no s'arriben a distingir.}
	\label{fig:2.3}
\end{figure}
...
\begin{table}[h]
	...
	\begin{tabular}{cccc}
		\toprule
		Intensitat (A) & Angle (rad) & Força (N) & Camp B (T) \\
		\midrule
