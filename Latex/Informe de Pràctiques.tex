...
Posteriorment, per l'estudi del règim transitori, muntarem altre circuit com ja hem explicat a la secció \ref{sec 3.2.2}. En concret reproduïrem el circuit de la figura \ref{fig:3.3} amb una resistència de $R = 180\Omega$, $L = 33$mH i $C=330$pF. Amb aquests valors calculem teòricament alguns paràmetres del circuit com la pseudo-frequencia, $\omega$, el pseudo període, $T$, i la freqüència, $f$, que hem de posar al generador per tal que $f=1/(10T)$:
...
\subsection{Règim permanent}
Una vegada s'han mesurat les freqüències de ressonància i de tall, els valors dels voltatges d'entrada i de la resistència just com s'explica a la secció \ref{expermento permanente} podem obtenir els següents valors experimentals directament de l'oscil·loscopi:
...
A la taula \ref{val exp trans} es troben els resultats de l'experiment. Com els valors són varien considerablement, l'anàlisi d'aquestes dades es farà, sobretot, a partir del percentatge d'error respecte als valors teòrics predits que s'adjunten a la taula \ref{error exp trans}. 
...
L'error de $f_1$ per a $R = 2660\Omega$ pot ser causat per la falta de presició en la selecció de freqüències de l'oscil·loscopi, ja que aquest pot donar valors amb molta precisió però nosaltres vam considerar que anar fent salts de 50Hz o de 100Hz era bona idea, ja que freqüències eren de l'ordre de milers d'hertzs. Ho vam considerar així perquè experimentalment tampoc s'aprecia un gran salt en $\abs{T_R(\omega)}$ en moure la freqüència de 100 Hz en 100 Hz (ho podem veure a la figura \ref{fig:3.4}. Això últim concorda amb la teoria, ja que $\abs{T_R(\omega)}$ a freqüències molt baixes o molt altes comparades amb $f_0$, com ho son $f_1$ i $f_2$, $\abs{T_R(\omega)}$, canvia molt lentament. Aquest fet es pot deduir veient la forma de l'equació \ref{eq:3.33} i tenint en compte que $\abs{T_R(\omega)}$ està acotat en $(0,1]$. Així, el canvi en $\abs{T_R(\omega)}$ per a aquests ordres de freqüència no és pràcticament apreciable. Això podria explicar que, en algunes freqüències, tinguem un error relativament alt però que per a aquestes mateixes freqüències l'error en $\abs{T_R(\omega)}$ no sigui molt important. 
...
És important observar que hem aconseguit reproduir perfectament els ordes de magnitud de totes les freqüències i veure com el guany en voltatge de la resistència va canviant amb aquestes, sent màxima en la freqüència de ressonància del sistema. També, gràcies als errors comesos hem pogut experimentar el diferent comportament del sistema per a diferents valors de la resistència, com hem pogut veure en els valors de $\abs{T_R(\omega)}$. \\

Ara, un cop els errors han estat discutits, podem comparar valors i veure com, tot i canviar la resistència, la freqüència de ressonància, $f_0$, és la mateixa per a les dues resistències, just com prediu la fórmula \ref{eq:3.19}, ja que aquesta, segons la teoria, no depèn de la resistència.  
Donat que $f_0$ no canvia, $\abs{T_R(\omega_0)}$ tampoc ho fa. De fet, per molt que la freqüència de ressonància canviï (per exemple canviant el condensador o la inductància), si recalculem $\abs{T_R(\omega_0)}$ per a aquesta nova freqüència, se seguiria complint que $\abs{T_R(\omega_0)} = 1$. Això és així per definició, tal i com hem deduït a la introducció de l'equació \ref{eq:3.18}.

Per contra, el que sí que podem notar que canvia en funció de la resistència que posem són les freqüències de tall. Aquestes depenen explícitament de la forma de $\abs{T_R(\omega)}$ i no d'on aquesta sigui màxima (tal i com passava amb $f_0$). Ara, \textcolor{red}{la freq de tall? o què és el que queda fixa??} queda fixada independentment de $R$. Això es pot veure bé les dades presentades. Com hem comentat, mentre que a $R = 274\Omega$, $f_1$ i $f_2$ quedaven molt a prop de $f_0$ per la forma del pic de $\abs{T_R(\omega)}$ (hem apreciat els seus canvis bruscos), en posar $R = 2660\Omega$, tot i que tampoc diríem que $f_1$ queda lluny de $f_0$, $f_2$ està bastant allunyat (de l'ordre del doble de freqüència). De fet, aquesta diferència de comportament no només la podem veure visualment a les figures \ref{fig:3.4} i \ref{fig:3.5}, sinó que també es pot quantificar amb un paràmetre anomenat ample de banda: 
\begin{eqnarray}
	\Delta\omega \equiv \omega_2 - \omega_1
\end{eqnarray}
Substituint els nostres valors experimentals podem calcular $\Delta f \equiv f_2 - f_1$ ($\Delta\omega /(2\pi)$):
...
Observem que en aquestes mesures hem comés molt poc error, fet que concorda amb el que s'ha vist abans. En tenir una resistència petita, el circuit es comporta de manera molt sensible sota un cavi en la freqüència, de manera que hem pogut mesurar amb molta exactitud i precisió el pseudo-període, ja que és aquell pel qual es sistema oscil·la en aquestes condicions.\\
\textcolor{red}{(Com a valor experimental no posem la pseudo-freqüència $\omega$, perque el que mesurem experimentalment el el pseudo-període, per lo que fem l'anàlisi a partir d'ell)  ESTO VA ASÍ TAL CUAL? O ES ANOTACIÓN TUYA???? Me parece random un paréntesis así}

\textcolor{red}{HE CORREGIDO HASTA AQUÍ. NO BORRES ESTO ASÍ MAÑANA ERIC SE LO PUEDE MIRAR}
...
\textcolor{red}{se pueden hacer más pequeñitos los gráficos o queda demasiado pequeño?}
