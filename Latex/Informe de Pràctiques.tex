...
\end{eqnarray*}
Això ens permet calcular certes propietats del circuit, com per exemple: $\text{T}_\text{R}(\omega)$:
...
\begin{table}[h]
	...
	\begin{tabular}{lccc} 
		...
		& $\omega$ (\text{rad/s}) & $T$ (\text{s}) & $f$ (\text{Hz})\\ 
		\midrule
		$\mathbf{R = 184\Omega}$ & - & (2,060 $\pm$ 0,020)$\cdot 10^{-5}$ & (4,854 $\pm$ 0,047)$\cdot 10^3$\\
		\bottomrule
	\end{tabular}
\end{table}
...
\begin{table}[h]
	...
	\begin{tabular}{lccc} 
		...
		& $\omega$ (\text{rad/s}) & $T$ (\text{s}) & $f$ (\text{Hz})\\ 
		\midrule
		$\mathbf{R = 184\Omega}$ & - & (0,7 $\pm$ 1,5)$\%$  & (0,7 $\pm$ 1,5)$\%$ \\
		\bottomrule
	\end{tabular}
\end{table}
...
\begin{equation}
	T_{s/e}=\frac{V_{sortida}}{V_{entrada}}
	\label{eq4:25}
\end{equation}
