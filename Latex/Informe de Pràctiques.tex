...
Podem veure que, clarament, dels dos experiments duts a terme, el primer (corresponent a l'estudi de la dependència de la força entre els fils en funció del corrent) és el que dóna un resultat de $\mu_0$ més proper al real. El fet que el segon experiment (corresponent a l'estudi de la dependència de la força entre els fils en funció de la separació entre aquests) presenti una desviació de més del doble que el darrer es pot justificar pel fet que per fer aquest càlcul hem hagut de determinar prèviament la constant de torsió del fil amb mesures efectuades amb la balança i posteriorment utilizar aquestes dades per correlacionar mes mesures amb la balança amb la força del camp magnètic per lo que l'error que tinguem en mesurar s'acomula y es propaga al error de $\mu_0$. A part, no només el error intrinsec del mètode es propaga, sinó que el posible error que tinguem per determinar $k$ també s'acumula a l'hora de fer l'ajust lineal dels punts de $F$ vs. $1/r$.
En general, es normal que el segón metode doni mes error que el primer, que es una mesura mes directa. Tot i aixó al primer mètode tenim un error raonable, com pot ser? Aixó es degut a la propia naturalesa de l'experiment, si recordem al principi per obtenir quina expresió obeeix el camp a causa d'un fil per on circula corrent, hem suposat que aquest fil era infinit, així el camp que genera es proporcional a $1/r$ en tots els punts, en concret sobre l'altre fil el camp es el mateix ja que son paralels, aixo pero no es cert, evidentment no tenim un fil infinit pero ni tan sols la longitudl ($L'$) del fil que crea el camp compleix que $L'\gg L$. Nosaltres realment tenim dos fils de igual longitud, per lo que als extrems del fil que crea el camp, aquest no està generant tant camp com estem suposant ni distribuit de la mateixa manera ja que no es infinit, aixó explica que ens sortís un valor de $\mu_0$ major que el que hauria, es un error intrinsec de l'experiment.
Encara que tinguem un cert error en la mesura de $\mu_0$, del que podem estar segurs es en que la força segueix el comportament de l'equació \ref{eq:2.9}, ja que l'ajust lineal te una $R^2$ considerablement propera a $1$.

Una ultima cosa a comentar es sobre un dels errors comesos en regressió de la que extraiem la constant que modela la força de torsió. Si ens fixem en l'equació \ref{eq2:10}, la regresio de la figura \ref{fig:2.4}, aquesta no hauria de tenir ordenada a l'origen, però si la te, això va ser un error al calibrar la mesura, molt posiblement vam començar amb voltes extres lo que indueix un error sistematic. Tot i així no ens importa molt ja que el que ens interesa es el pendent i això segueix sent correcte.
...
Una vegada muntat el circuit de la figura \ref{fig:3.2} amb $R=2660$ $\Omega$, $L=22$ mH, $C=15$ nF i una font amb un voltatge d'entrada tipus $V(t) = V_{max}\sin(\omega t)$, la freqüència de ressonància mesurada experimentalment és
\begin{equation*}
	f_0^{exp} = (8960 \pm 1) \text{ s$^{-1}$}.
\end{equation*}
Si comparem aquest valor mesurat amb el valor teòric que s'obté a partir de l'equació \eqref{eq:3.41}
\begin{equation*}
	f_0^{teo} = 8761 \text{ s$^{-1}$},
\end{equation*}
podem veure que els valors són pràcticament iguals, com era d'esperar. Fent aquesta mateixa mesura, però canviant la resistència per una de $R=274$ $\Omega$, obtenim una freqüència
\begin{equation*}
	f_0^{exp} = (8860 \pm 1) \text{ s$^{-1}$},
\end{equation*}
tot i que, teòricament, la freqüència de ressonància és independent de la resistència del circuit i, per tant, no hauria de canviar.
...
\begin{table}[h]
	...
	\caption{Guanys experimentals i teòrics a la freqüència de ressonància del sistema.}
	\begin{tabular}{lcc}
		\toprule
		& $\abs{T_R(\omega_0)}(R=2660 \text{ $\Omega$})$ & $\abs{T_R(\omega_0)}(R=274 \text{ $\Omega$})$ \\ 
		\midrule
		\textbf{Teòric} & 1 & 1 \\ 
		\textbf{Experimental} & $1,000\pm 0,008$ & $0,877 \pm 0,006$\\ 
		\bottomrule
	\end{tabular}
	\label{tab:3.1}
\end{table}
...
Mesurem també les freqüències de tall del circuit RLC per poder trobar, a partir d'elles, l'amplada de banda del sistema. Si resolem el sistema d'equacions donat per \eqref{eq:3.38} i \eqref{eq:3.39}, podem trobar que
\begin{equation}
	f_1^{teo} = \frac{1}{4\pi}\left(-\frac{R}{L} + \sqrt{\left(\frac{R}{L}\right)^2 + 4\frac{1}{LC}}\right),
\end{equation}
\begin{equation}	
...
\begin{chapterabstract}
	En aquesta pràctica estudiem el comportament dels transformadors reals i analitzem la dependència que pot tenir la geometria del nucli de material ferromagnètic sobre la idealitat d'aquest, determinada pel coeficient d'acoblament.
	 \textbf{ACABARRRRRRRRRRRRRRR}
\end{chapterabstract}
...
Cal destacar que la primera configuració proposada per nosaltres també presenta un valor de $k$ molt proper a la idealitat (0,9535 ± 0,00195), tot i ser lleugerament inferior al de la configuració d (0,9562 ± 0,00196). Malgrat això, considerem que aquesta primera configuració hauria de ser la millor des del punt de vista geomètric, ja que la seva forma s’ajusta especialment bé al recorregut natural de les línies del camp magnetic generades per la bobina primària. Per aquest motiu, l’hem escollida com a base per a les mesures posteriors demanades.

Per finalitzar l'anàlisi, podem veure la importància d’introduir un element ferromagnètic per minimitzar la pèrdua de flux. Si ens fixem en la configuració 0, que correspon al cas sense cap element ferromagnètic, la constant d’acoblament és de $k = 0,0419 \pm 0,00017 $, el valor més baix amb diferència respecte a la resta de configuracions. Això indica que una gran part del flux magnètic no arriba a la bobina secundària, evidenciant una desviació notable del comportament ideal i una gran pèrdua de voltatge.

Per a la segona part d'aquesta secció usarem la primera configuració proposada (REF) com ja hem comentat anteriorment. Les mesures experimentals dels voltatges d'entrada i de sortida per a les diferents combinacions de bobines es troben a la taula següent. 
\begin{table}[H]
	\centering
	\renewcommand{\arraystretch}{1.2}
	\caption{Valors experimentals dels voltatges d'entrada $(V_1)$ i de sortida $(V_2)$ i la semblança amb la idealitat per a diferents combinacions de bobines usant la primera configuració proposada per nosaltres.}
	\begin{tabular}{lcccccccc}
		\toprule
		$n_1$ & $n_2$ & $n_2/n_1$ & $V_1 (V)$ & $V_2 (V)$ & $V_2/V_1$ & Idealitat (\%) & Error (\%) \\
		\midrule
...
Com veiem en el desenvolupament teòric \eqref{eq4:11} idealment el guany de voltatge hauria de ser igual al quocient $\alpha$, segons la llei del transformador ideal. L'última columna de la taula mostra el percentatge de semblança entre el valor experimental i el valor teòric ideal. Tot i que cap dels valors és exactament del $100\%$, degut a les pèrdues reals del sistema, podem afirmar que la llei es compleix amb bona aproximació per a totes les combinacions analitzades.

Cal destacar algunes tendències observades. En general, a mesura que augmenta la diferència relativa entre $n_1$ i $n_2$, la desviació respecte del comportament ideal també augmenta. Això pot explicar-se pel fet que les relacions molt desiguals entre les dues bobines afavoreixen les pèrdues per flux dispers i un pitjor acoblament magnètic.
...
Aquestes observacions són coherents amb el comportament esperat d’un transformador real, en el qual les pèrdues per flux dispers, histèresi i escalfament impedeixen que la relació \( \frac{V_2}{V_1} = \frac{n_2}{n_1} \) es compleixi exactament.

\subsection{Estudi de les intensitats i voltatges d'entrada i de sortida}
En aquesta secció trobem la relació entre les reactàncies d'una bobina de 400 voltes i una de 800 voltes a circuit obert. Els valors mesurats experimentalment dels voltatges i corrents d'entrada es poden trobar a la taula següent. 
...
La reactància es calcula com $X = V / I $. 
La inductància d'una bobina és proporcional al quadrat del nombre de voltes que té. Com la reactància és directament proporcional a la inductància, aquesta també ha de ser proporcional al quadrat del nombre de voltes de la bobina. Si volem trobar la relació entre dues reactàncies, la relació ha de ser el quadrat de la relació entre el nombre de voltes de cada bobina, és a dir.

\begin{equation*}
	\begin{array}{c}
		L \propto n^2 \Rightarrow X \propto n^2 \\
		\frac{X_{800}}{X_{400}} = \left( \frac{800}{400} \right)^2 = 4
	\end{array}
\end{equation*}
...
Per a l’última part d’aquesta secció, connectem un circuit al secundari, sempre amb una bobina de 400 voltes, i estudiem els corrents d’entrada i de sortida. A la taula següent es mostren les mesures experimentals per a les combinacions amb 400 i 800 voltes al primari, i una combinació proposada per nosaltres amb 3200 voltes. La configuració usada és la que es demana al guió, que és la \textbf{d} \textcolor{red}{posaré ref quan es posi la configuracio d a metodologia, q falta!!}. 
...
Un cop calculat el guany, el comparem amb el valor teòric. la següent taula presenta els valors experimental i teòrics calculats tenint en compte en quin cas ens trobem. També es troba l'error respecte el valor teòric.  
...
\begin{table}[H]
	...
	\begin{tabular}{llcccccc}
		\toprule
		$n_1/n_2$ & Z & Cas & $X$ & $V_2/V_1$ exp & $V_2/V_1$ teòric & Error (\%) & $u_e$ \\
		\midrule
		400/400 & 1000 & $Z \gg X$ & 100,42 & 0,9576 $\pm$ 0,0019 & 0,956 $\pm$ 0,002 & 0,15 & 0,32 \\
		& 100  & $Z \-- X$ & 100,42 & 0,9203 $\pm$ 0,0019 & 0,956 $\pm$ 0,002 & 3,75 & 0,31 \\
		...
		& 100  & $Z \-- X$ & 385,2 & 1,559 $\pm$ 0,003 & 0,4781 $\pm$ 0,0016 & 226,13 & 1,24 \\
		& 10   & $Z \ll X$ & 385,2 & 0,4353 $\pm$ 0,0016 & 0,369 $\pm$ 0,011 & 18,10 & 3,57 \\
		\midrule
		3200/400 & 1000 & $Z \-- X$ & 6295,5 & 5,437 $\pm$ 0,016 & 0,1195 $\pm$ 0,0004 & 4448,97 & 20,33 \\
...
\begin{table}[H]
	...
	\begin{tabular}{llcccccc}
		\toprule
		$n_1/n_2$ & Z & Cas & $k$ & $V_2/V_1$ & $k'$ & $V_2/V_1$ amb $k'$ & Variació relativa del guany (\%) \\
		\midrule
		400/400 & 1000 & $Z \gg X$ & 0,9562 & 0,95615 & 0,9500 & 0,9500 & 0,64 \\
		& 100  & $Z \-- X$ & 0,9562 & 0,95615 & 0,9500 & 0,9500 & 0,64 \\
		& 10   & $Z \ll X$ & 0,9562 & 0,72446 & 0,9500 & 0,6788 & 6,30 \\
		\midrule
		800/400 & 1000 & $Z \gg X$ & 0,9562 & 0,47808 & 0,9500 & 0,4750 & 0,64 \\
		& 100  & $Z \-- X$ & 0,9562 & 0,47808 & 0,9500 & 0,4750 & 0,64 \\
		& 10   & $Z \ll X$ & 0,9562 & 0,36858 & 0,9500 & 0,3463 & 6,05 \\
		\midrule
		3200/400 & 1000 & $Z \-- X$ & 0,9562 & 0,11952 & 0,9500 & 0,1188 & 0,64 \\
...
Cal mencionar que, tot i que les dues configuracions amb $3200/400$ i resistències de $100\,\Omega$ i $10\,\Omega$ pertanyen al mateix règim $Z \ll X$, només en el cas de $10\,\Omega$ el guany teòric mostra una dependència clara amb $k$. Aquest comportament es pot explicar analitzant l’expressió del guany (equació~\ref{eq4:22}), on apareix un terme amb $k$, el denominador sent:

\[
[ Z^2 + X^2 \alpha^4 (k^2 - 1)^2 ]^{1/2}
\]

En aquestes dues configuracions, la reactància $X = 6295{,}5\,\Omega$ i el quocient d’espira $\alpha = 0{,}125$ es mantenen constants. Per tant, l’únic paràmetre que canvia és la resistència $Z$. Si comparem els quadrats de les resistències, obtenim $Z^2 = 100$ per al cas de $10\,\Omega$ i $Z^2 = 10\,000$ per al cas de $100\,\Omega$.

En el primer cas ($Z = 10\,\Omega$), el valor de $Z^2$ és del mateix ordre de magnitud que el terme amb $k$, de manera que aquest últim té un pes significatiu en el denominador i el guany passa a dependre clarament del valor de $k$. En canvi, en el cas de $100\,\Omega$, el terme $Z^2$ domina clarament al denominador, fent que la contribució del terme amb $k$ sigui negligible. Aquesta és, doncs, l’excepció que no segueix la tendència observada.


De fet, a mesura que el nombre de voltes del primari augmenta respecte del secundari, el quocient $\alpha$ es fa molt petit, i en conseqüència $\alpha^4$ es fa gairebé nul. Tot i que la reactància $X$ també augmenta (ja que $X \propto N_1^2$), aquest creixement no és suficient per compensar la pèrdua deguda a $\alpha^4$ dins el terme $X^2 \alpha^4 (k^2 - 1)^2$, que esdevé totalment negligible.

A més, aquest augment de $X$ permet fer servir resistències relativament grans i continuar estant dins el règim $Z \ll X$. Això fa que el terme $Z^2$ del denominador domini completament i anul·li l’efecte de $k$, independentment de la seva variació.

En resum, com més gran és la diferència entre el nombre de voltes del primari i del secundari, més fàcil és que el coeficient d’acoblament $k$ deixi de tenir impacte en el guany.

Ara si ens fixem ens els altres casos $Z \gg X$ (\ref{eq4:20}) o $Z \sim X$ (\ref{eq4:24}), el guany depèn linealment de $k$ , i per tant, petites variacions en aquest coeficient només provoquen canvis mínims en el guany teòric. Això explica per què els percentatges de variació són molt menors en aquests casos.
