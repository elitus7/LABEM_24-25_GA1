...
\begin{figure}[H]
	\centering
	\includegraphics[width=0.75\linewidth]{R2660.png}
...
\begin{figure}[h]
	\centering
	\includegraphics[width=0.75\linewidth]{R274.png}
...
\begin{table}[H]
	...
	\begin{tabular}{lccc}
		...
		\midrule
		$\mathbf{R = 2660\Omega}$ & 19240 $\pm$ 730 & 19503,0 $\pm$ 1,4 & 1,35 $\pm$ 3,8\\ 
...
\textcolor{red}{podria cuadrar los resultados un poco mas si calculara la R tal que los resultados cuadren y así por diferencia calcular la R del cable y verificar si cuadra con la R de un cable tipico? Podria recalcularse todo para este nuevo valor de R y hacer un mini anàlisi. Estaria guapisismo y explicaria el prqueño error en las medidas, pq es una idealizacion que en un principio no considerabamos}\\
...
\subsection{Règim transitori}
Una vegada muntat el circuit de la figura \ref{fig:3.3} amb $R$, $L$ i $C$ de $184\Omega$, $33mH$ i $330pF$ respectivament podem presentar els resultats. 

\textcolor{red}{$AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA$}

Per finalitzar, trobarem el valor experimental de $\lambda$ i el compararem amb el valor teòric.

El valor teòric s'obté a partir de l'equació \eqref{eq:3.24}, que pel nostre cas amb una resistència de $R = 1448\,\Omega$ i autoinductància de $L = 33\,\text{mH}$,   obtenim

\begin{equation}
	\lambda = \left( 21939{,}40 \pm 15{,}15 \right) \, \text{s}^{-1}
\end{equation}

Per trobar el valor experimental de $\lambda$, representem la recta corresponent a l’equació \eqref{eq:3.28}, on la pendent serà $ -\lambda$. 

Tot i que el valor de $E$ és conegut perquè correspon al voltatge d’entrada aplicat pel generador, hem optat per determinar-lo de manera gràfica. En la pràctica experimental, poden aparèixer factors que fan que el valor real percebut pel circuit no coincideixi exactament amb el valor del generador. Aquests factors poden ser pèrdues de senyal, el soroll de fons  o una resposta no ideal del sistema. L'obtenim fent el límit de l’equació \eqref{eq:3.27}, quan $t \to \infty$:

\begin{equation}
	\lim_{t \to \infty} V_C(t) = E
\end{equation}

Aquest límit s’ha determinat gràficament a partir de la gràfica $\ref{fig:3.6}$, construïda amb les dades de la taula $\ref{tab:3.7}$.
...
\begin{table}[H]
	...
	\begin{minipage}{0.48\textwidth}
		\centering
		\includegraphics[width=\linewidth]{grafica.png} 
		\captionof{figure}{Gràfica de $\ln|V_C(t_n) - E|$ en funció del temps $t$.}
		\label{fig:3.7}
	\end{minipage}
\end{table}


La regressió lineal de la gràfica $\ref{fig:3.7}$ ens queda $y = -25091x + 1{,}3074 $, obtenint $\lambda = 25091\, \text{s}^{-1} $. 

Finalment, si comparem el valor experimental amb el valor teòric, obtenim un error relatiu del $14{,}37\% $, que es pot considerar acceptable tenint en compte les incerteses experimentals i el fet que no tenim un circuit ideal. 
