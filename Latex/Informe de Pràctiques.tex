...
\begin{figure}[h]
	...
	\caption{Força entre els dos fils en funció de la intensitat del corrent al quadrat. Dades de l'ajust lineal: $y=mx+n$, amb $m = (1,0\pm4,5)\cdot 10^{-5}$ N$\cdot$A$^{-2}$ i $n = (0,3\pm4,0)\cdot 10^{-6}$ N. Coeficient de correlació $R^2=0,997$. Notem que, en alguns punts, les barres d'error corresponents a l'eix de les $x$ són tan petites que no s'arriben a distingir.}
	\label{fig:2.3}
\end{figure}
...
Podem veure que, clarament, dels dos experiments duts a terme, el primer (corresponent a l'estudi de la dependència de la força entre els fils en funció del corrent) és el que dóna un resultat de $\mu_0$ més proper al real. El fet que el segon experiment (corresponent a l'estudi de la dependència de la força entre els fils en funció de la separació entre aquests) presenti una desviació de més del doble que el darrer es pot justificar pel fet que per fer aquest càlcul hem hagut de determinar prèviament la constant de torsió del fil; si per determinar $k$ hem comés algun tipus d'error, per petit que sigui, aquest s'haurà propagat a l'hora de fer l'ajust lineal dels punts de $F$ vs. $1/r$.

\textbf{SEGUIR COMENTANT COSES I TAL. ideas per dir:}
\begin{itemize}
	\item Les dues R's són "bones" (revisar conveni per quants decimals posar).
	\item La recta de $F = k\theta$ no passa per l'origen exactament $\Rightarrow$ rarete
	\item Com a mínim els resultats són de l'ordre del valor real!
\end{itemize}
