...
\begin{equation}
	...
	\label{eq2:10}
\end{equation}
Notem que hem assumit que l'angle de torsió $\theta$ i la força $F$ són proporcionals, mitjançant una constant recuperadora $k$. Això que té sentit si pensem en la llei de Hook, que, recordem, ens diu que la força recuperadora que fa una molla és
\begin{equation}
	F = k\Delta x
\end{equation} 
...
\begin{equation}
	k\theta = \frac{\mu_0LI^2}{2\pi}\frac{1}{r}.
\end{equation}
...
\end{equation}
Ara, a partir del valor d'aquesta constant, podem correlacionar les voltes necessàries del dial per equilibrar la balança en cada una de les configuracions a diferents distàncies, amb la força magnètica que produeix el corrent fixat de $I = (5,55 \pm 0,01)$ A que apliquem. Si fem una representació de la força entre els fils vs. $1/r$, que teòricament hauria de ser una recta, ja que:
\begin{eqnarray}
...
Tal com se'ns indica a la metodologia de la pràctica, l'orientació de la balança ha de ser tal que la intensitat i la direcció nord-sud de la Terra siguin perpendiculars (és a dir, alineats amb la direcció est-oest). Amb aquesta configuració, notem que la component del camp magnètic terrestre que mesurem és l'horitzontal, ja que, si ens fixem en l'equació \eqref{eq2:6}, $\vec{B} \cdot \text{d}\vec{l} = B\cdot\text{d}l\cdot\cos(\theta)$, sent $\theta$ l'angle entre els dos vectors. Observem que si $\vec{B}$ i d$\vec{l}$ són vectors perpendiculars, és a dir $\theta = 90$ º o $\theta = 270$ º, el seu producte escalar s'anul·la. Per tant, en situar els conductors perpendiculars a la direcció nord-sud, aquests es troben alineats amb la component horitzontal (est-oest) de la Terra, fent que així el producte escalar sigui diferent de zero. ERIC DICE QUE NO ESTÁ BIEN. YA LO ESCRIBIRÁS TÚ.
...
ES VEU Q ELS VALORS ESTAN AL REVES LOL !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
(s'ha de comentar, quan ho arregli, otro dia, q no puc mes)
...
Els valors experimentals calculats per a cada cas es troben a la taula següent. 
